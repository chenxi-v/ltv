...
interface AutoAspectImageProps {
  src: string;
  alt: string;
  className?: string;
...
const AutoAspectImage: React.FC<AutoAspectImageProps> = ({ src, alt, className = '', onRatioDetected }) => {
  ...
  useEffect(() => {
    ...
    img.onerror = () => {
      setDimensions({ width: 2, height: 3, ratio: 2/3 });
      setLoaded(true);
    };
    img.src = src;
  }, [src, onRatioDetected]);
  ...
  const getAspectClass = () => {
    if (dimensions.ratio > 1.7) {
      return 'aspect-video'; // 超宽屏 > 16:9
    } else if (dimensions.ratio > 1.4) {
      return 'aspect-[4/3]'; // 4:3 或略宽
    } else if (dimensions.ratio > 0.7) {
      return 'aspect-[2/3]'; // 标准海报比例
    } else {
      return 'aspect-[2/3]'; // 竖版图片
    }
  };

  return (
    <div className={`w-full ${getAspectClass()} relative`}>
      <img
        src={src}
        alt={alt}
...
// 图片比例切换
const [imageAspectRatio, setImageAspectRatio] = useState<'2/3' | '16/9' | 'auto'>('auto');

// 图片尺寸缓存
const [imageDimensions, setImageDimensions] = useState<Record<string, { width: number; height: number; ratio: number }>>({});
...
// 检测图片尺寸并设置比例
const detectImageRatio = useCallback(async (url: string): Promise<{ width: number; height: number; ratio: number }> => {
  // 如果已经缓存过，直接返回
  ...
  return new Promise((resolve) => {
    ...
    img.onerror = () => {
      // 如果加载失败，默认返回 2/3 比例
      const dimensions = { width: 2, height: 3, ratio: 2/3 };
      setImageDimensions(prev => ({ ...prev, [url]: dimensions }));
      resolve(dimensions);
    };
    img.src = url;
            ...
            <div className='px-5 py-4 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between'>
              ...
              <div className='flex items-center gap-3'>
                ...
                <div className='flex items-center gap-1 p-1 rounded-lg bg-gray-100 dark:bg-gray-700'>
                  <button
                    onClick={() => setImageAspectRatio('2/3')}
                    className={`px-2 py-1 text-xs rounded-md transition-colors ${imageAspectRatio === '2/3' ? 'bg-white dark:bg-gray-600 text-blue-600 dark:text-blue-400 shadow-sm' : 'text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-200'}`}
                    title="4:3 比例"
                  >
                    4:3
                  </button>
                  <button
                    ...
                    className={`px-2 py-1 text-xs rounded-md transition-colors ${imageAspectRatio === '16/9' ? 'bg-white dark:bg-gray-600 text-blue-600 dark:text-blue-400 shadow-sm' : 'text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-200'}`}
                    title="16:9 比例"
                  >
                    16:9
                  </button>
                  <button
                    onClick={() => setImageAspectRatio('auto')}
                    className={`px-2 py-1 text-xs rounded-md transition-colors ${imageAspectRatio === 'auto' ? 'bg-white dark:bg-gray-600 text-blue-600 dark:text-blue-400 shadow-sm' : 'text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-200'}`}
                    title="自动比例"
                  >
                    自动
                  </button>
            ...
            <div className='p-5 space-y-5'>
              ...
              <div>
                {loadingItems ? (
                  ...
                  <>
                    <div className='grid gap-3 grid-cols-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-5'>
                      {filteredAndSorted.map((item, index) => (
                        ...
                        >
                          ...
                          <div className={`${imageAspectRatio === '2/3' ? 'aspect-[2/3]' : ''} ${imageAspectRatio === '16/9' ? 'aspect-[16/9]' : ''} ${imageAspectRatio === 'auto' ? '' : ''} bg-linear-to-br from-gray-100 via-gray-50 to-gray-100 dark:from-gray-700 dark:via-gray-800 dark:to-gray-700 overflow-hidden relative`}>
                            {item.poster ? (
                              <AutoAspectImage 
                                src={item.poster} 
                                alt={item.title}
                                className='w-full h-full object-cover group-hover:scale-110 transition-transform duration-500'
                                onRatioDetected={(ratio) => detectImageRatio(item.poster)}
                              />
                            ) : (
                  ...
                  <div className='grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-5 md:gap-6'>
                    ...
                    <div className='md:col-span-1'>
                      <div className='md:sticky md:top-0'>
                        {previewItem?.poster ? (
                          <div className={`relative rounded-xl md:rounded-2xl overflow-hidden shadow-lg md:shadow-2xl border border-gray-200 dark:border-gray-700 md:border-2 group max-w-[200px] mx-auto md:max-w-none ${imageAspectRatio !== 'auto' ? (imageAspectRatio === '2/3' ? '' : 'aspect-video') : ''}`}>
                            <AutoAspectImage 
                              src={previewItem.poster} 
                              alt={previewItem.title}
                              className='group-hover:scale-105 transition-transform duration-300'
                              onRatioDetected={(ratio) => detectImageRatio(previewItem.poster)}
                            />
                          ...
                          <div className={`w-full max-w-[200px] mx-auto md:max-w-none ${imageAspectRatio === '2/3' ? 'aspect-[2/3]' : 'aspect-video'} bg-linear-to-br from-gray-100 to-gray-200 dark:from-gray-700 dark:to-gray-800 rounded-xl md:rounded-2xl flex items-center justify-center border border-gray-200 dark:border-gray-700 md:border-2`}>
