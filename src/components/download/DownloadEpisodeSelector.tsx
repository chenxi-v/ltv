          ...
          {pageCount > 1 && (
            <div className='flex items-center gap-2 sm:gap-4 px-4 sm:px-6 py-2 sm:py-3 border-b border-gray-200 dark:border-gray-700 shrink-0'>
              <div className='flex-1 overflow-x-auto'>
                <div className='download-category-container relative inline-flex bg-gray-200/60 rounded-full p-0.5 sm:p-1 dark:bg-gray-700/60 backdrop-blur-sm min-w-max'>
                  {/* 滑动的白色背景指示器 */}
                  <div
                    className='absolute top-0.5 bottom-0.5 sm:top-1 sm:bottom-1 bg-white dark:bg-gray-500 rounded-full shadow-sm transition-all duration-300 ease-out'
                    style={{
                      left: `${Array.from(document.querySelectorAll('.download-category-button')).length > displayPage ? (document.querySelectorAll('.download-category-button')[displayPage] as HTMLElement)?.getBoundingClientRect().left - (document.querySelector('.download-category-container') as HTMLElement)?.getBoundingClientRect().left || 0 : 0}px`,
                      width: `${Array.from(document.querySelectorAll('.download-category-button')).length > displayPage ? (document.querySelectorAll('.download-category-button')[displayPage] as HTMLElement)?.getBoundingClientRect().width || 0 : 0}px`,
                    }}
                  />
                  ...
                  {categories.map((label, idx) => {
                    ...
                    return (
                      <button
                        key={label}
                        className={`download-category-button relative z-10 px-3 sm:px-4 py-1.5 sm:py-2 text-xs sm:text-sm font-medium rounded-full transition-all whitespace-nowrap active:scale-95
                        ${
                          isActive
              ...
              {/* 向上/向下按钮 */}
              <button
                className='shrink-0 w-8 h-8 rounded-md flex items-center justify-center text-gray-700 hover:text-gray-900 hover:bg-gray-100 dark:text-gray-400 dark:hover:text-gray-100 dark:hover:bg-gray-600 transition-colors active:scale-95'
